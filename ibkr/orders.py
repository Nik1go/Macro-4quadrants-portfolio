...
from .config import (
    ...
    LIVE_ASSETS_DATA_PATH, ACCOUNT_ID
)
...
class OrderManager:
    ...
    def _get_fallback_price(self, asset_name: str) -> Optional[float]:
        ...
        try:
            if asset_name in ['USD_JPY', 'USD_EUR']:
                ...
                live_price_col = f'{asset_name}_ibkr_live_price'
                if os.path.exists(LIVE_ASSETS_DATA_PATH):
                    df_live = pd.read_csv(LIVE_ASSETS_DATA_PATH)
                    if live_price_col in df_live.columns:
                        series = pd.to_numeric(df_live[live_price_col], errors='coerce').dropna()
                        if not series.empty:
                            price = float(series.iloc[-1])
                            logger.info(f"Fallback selected for {asset_name}: {price:.4f} (IBKR Live-Compatible Data)")
                            return price
                ...
                if asset_name in df.columns:
                    price = float(df[asset_name].iloc[-1])
                    logger.warning(f"Fallback selected for {asset_name}: {price:.4f} (Model Proxy Data, not IBKR ticker)")
                    return price
            return None
        except Exception as e:
    ...
    def calculate_rebalance_orders(
        ...
        for asset_name in all_assets:
            ...
            # For ETFs quoted outside the account base currency, convert the
            # instrument price before sizing. Otherwise SXRM/USD would be sized
            # as if the quote were EUR.
            asset_currency = details.get('currency', base_currency)
            if not is_forex:
                if asset_currency != base_currency:
                    rate_to_base = pm.get_exchange_rate(asset_currency, base_currency)
                    if rate_to_base <= 0:
                        logger.error(f"❌ Could not determine exchange rate {asset_currency}->{base_currency}")
                        continue
                    price_in_base = price * rate_to_base
                else:
                    price_in_base = price
                target_shares_magnitude = int(target_value_in_base / price_in_base)
                cross_rate_to_base = price_in_base
            else:
                if asset_currency == base_currency:
    ...
    def _wait_for_order_fill(self, trade, expected_shares: int,
                             timeout_seconds: int = ORDER_FILL_TIMEOUT_SECONDS) -> Dict[str, any]:
        """Wait until IBKR confirms a fill or the order reaches a terminal state."""
        ...
        while time.time() < deadline:
            ...
            if status in SUCCESS_ORDER_STATUSES and filled >= expected_shares:
                return {
                    'status': status,
                    'filled': filled,
                    'remaining': remaining,
            ...
            if status in FINAL_ORDER_STATUSES and status not in SUCCESS_ORDER_STATUSES:
                return {
                    'status': status,
                    'filled': filled,
                    'remaining': remaining,
                    ...
                    'error': f'Order reached terminal status {status}',
                }
        ...
        return {
            ...
            'confirmed_fill': False,
            'error': f'Order not filled before {timeout_seconds}s (status={last_status})',
        }
    ...
    def execute_orders(
        ...
        for order in orders:
            try:
                ...
                # Place order and wait for the real fill confirmation.
                trade = self.ib.placeOrder(contract, ib_order)
                fill_info = self._wait_for_order_fill(trade, order.shares)
                ...
                order_result = {
                    ...
                    'remaining': fill_info['remaining'],
                    'avgFillPrice': fill_info['avgFillPrice'],
                    'confirmed_fill': fill_info['confirmed_fill'],
                    'order_id': trade.order.orderId,
                    'error': fill_info['error'],
                }

                if fill_info['confirmed_fill']:
                    logger.info(
                        f"✅ Filled: {order.action} {order.shares} {order.symbol} "
                        f"@ {fill_info['avgFillPrice']:.4f}"
                    )
                    results['executed'].append(order_result)
                else:
                    logger.error(
                        f"❌ Not confirmed: {order.action} {order.shares} {order.symbol} "
                        f"- Status: {fill_info['status']} Filled: {fill_info['filled']} "
                        f"Error: {fill_info['error']}"
                    )
                    results['failed'].append(order_result)
