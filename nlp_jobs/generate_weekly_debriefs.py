    ...
    with path.open("r", encoding="utf-8") as handle:
        for line in handle:
            ...
            if line:
                ...
                records[record["period_end"]] = record
    return records
...
def system_prompt():
    return (
        "Tu es un analyste macro-financier prudent pour un projet de recherche quantitatif. "
        "Tu expliques en francais clair, sans promesse de performance et sans conseil d'investissement personnalise. "
        "Tu relies les donnees de l'algo, les nouvelles fournies quand elles existent, le quadrant economique et l'etat de la strategie. "
        "Si aucune news n'est fournie, tu dois le dire et ne jamais inventer d'evenement recent."
    )
...
def user_prompt(payload):
    schema = {
        ...
        "available_information": {"algo_data": ["string"], "news_data": ["string"], "missing_information": ["string"]},
        "nlp_signal": {
            "risk_on_score": "number between -1 and 1",
            "growth_score": "number between -1 and 1",
            "inflation_pressure_score": "number between -1 and 1",
            "policy_risk_score": "number between -1 and 1",
            "confidence": "number between 0 and 1",
            "suggested_use": "shadow_only | reduce_risk | add_risk | no_change",
            "rationale": "string",
        },
        "forward_view": {"base_case_next_week": "string", "bull_case": "string", "bear_case": "string", "watchlist": ["string"]},
        "key_points": ["string", "string", "string"],
        "risks": ["string", "string", "string"],
        "allocation_comment": "string",
        "markdown": "string",
    }
    return "Redige un debrief hebdomadaire pour la strategie Macro 4 Saisons.\nRetourne uniquement un JSON valide respectant cette structure:\n" + json.dumps(schema, ensure_ascii=False, indent=2) + "\n\nDonnees disponibles:\n" + json.dumps(payload, ensure_ascii=False, indent=2)


def call_openrouter(api_key, model, payload):
    import requests
    ...
    body = {"model": model, "temperature": 0.2, "max_tokens": 1800, "response_format": {"type": "json_object"}, "messages": [{"role": "system", "content": system_prompt()}, {"role": "user", "content": user_prompt(payload)}]}
...
def main():
    ...
    for week_start, week_end in week_periods:
        ...
        if args.dry_run:
            ...
            generated += 1
            continue
        content, usage = call_openrouter(api_key, model, payload)
        model_result = parse_model_content(content)
