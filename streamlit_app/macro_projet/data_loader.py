...
def load_data():
    ...
    try:
        data['backtest_ibkr_live'] = pd.read_csv(f"{base_dir}/backtest_results/backtest_ibkr_live_timeseries.csv", parse_dates=['date'])
    except:
        data['backtest_ibkr_live'] = None

    try:
        data['backtest_ibkr_live_stats'] = pd.read_csv(f"{base_dir}/backtest_results/backtest_ibkr_live_stats.csv")
    except:
        data['backtest_ibkr_live_stats'] = None

    try:
        data['backtest_ibkr_live_mapping'] = pd.read_csv(f"{base_dir}/backtest_results/backtest_ibkr_live_mapping.csv")
    except:
        data['backtest_ibkr_live_mapping'] = None
    ...
    try:
        from ibkr.tracking_audit import build_tracking_audit
        data['ibkr_tracking_audit'] = build_tracking_audit(base_dir)
    except Exception:
        data['ibkr_tracking_audit'] = None
